...
public class UserView {
	...
	private void multiInsertUser()throws Exception{
		...
		for(int i =0;i<input;i++) {
			...
			while(true) {
				...
				System.out.println("이미 사용중인 아이디니까.다시 입력해봐");
			}
			
			//pw, name 입력받기
			

			System.out.print((i+1)+" 번째PW : ");
	...
	private void insertUser2()throws Exception {
		...
		//서비스 호출(INSERT) 후 결과를 반환(int,결과행의 개수)받기
	...
	/**ID, PW가 일치하는 회원(SELECT)이 있을 경우 이름 수정(UPDATE)
	 * 
	 */
	private void updateName()throws Exception {
		
		System.out.println("\n===ID, PW가 일치하는 회원(SELECT)이 있을 경우 이름 수정\n===");
		...
		//위에서 조회된 회원의(userNo) 이름 수정 
		//서비스를 호출(UPDATE)후 결과 반환받기
